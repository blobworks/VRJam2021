...
public class OVRGrabber : MonoBehaviour
{
    ...
    protected virtual void GrabBegin()
    {
		...
		foreach (OVRGrabbable grabbable in m_grabCandidates.Keys)
        {
            ...
            if (!canGrab || !grabbable.isGrabbable)  // grabbable added by bw
            {
                continue;
            }
    ...
    protected void GrabbableRelease(Vector3 linearVelocity, Vector3 angularVelocity)
    {
        ...
        // new GameObject() created to overcome DDOL issue on drop
        if(m_parentHeldObject) 
        {
            // GameObject tempParent = new GameObject(); 
            
            // m_grabbedObj.transform.parent = tempParent.transform;
            m_grabbedObj.transform.parent = null;

            // Destroy(tempParent); 
            SceneManager.MoveGameObjectToScene(m_grabbedObj.gameObject, SceneManager.GetActiveScene()); 
        }
